...
function AddGames() {
    ...
    const { register, handleSubmit, formState: { errors }, setError, setValue, reset, control, trigger, setFocus, clearErrors, watch, getValues } = useForm({
        defaultValues: {
            gameFormData,
            priceFormData
        }
    ...
    const priceInput = (data) => {
        ...
        // 刪除 property 欄位
        delete newGame.property;

        finalData(newGame, finalRecords)
    };

    // 最後新增或編輯時的axios
    const finalData = async (games, prices) => {
        // 先新增遊戲取得ID
        let game_id = 0;
        try {
    ...
    const getStoreGameInfo = async () => {
        try {
            const res = await axios.get(`${baseApi}/gamesData/${game_id}`);
            setGameFormData(res.data);
        } catch (error) {
            console.error(error);
        }
    }
                            ...
                            <form onSubmit={(e) => onSubmit(e)}>
                                ...
                                <div className="p-6 fw-bold fs-h6 border-top border-nature-90  ">
                                    <div className="d-flex justify-content-end">
                                        <button className="btn bg-nature-60 text-white me-3">返回列表</button>
